...
class free_box extends WP_Widget {
	...
	function widget($args, $instance){
		...
		if( $instance['link'] == '' ){
			$title = "<span class='bg_color_{$instance['cor']}'>{$instance['titulo']}</span>";
		}
		else{
			$title = "<a href='{$instance['link']}' class='bg_color_{$instance['cor']}'>{$instance['titulo']}</a>";
		}
		...
		<div class="sidebar_box rss_box">
			...
			<div class="sidebar_box_content">
				...
				if( !empty($instance['video']) ){
					...
					$embed = $wp_embed->shortcode( $atts, $instance['video'] );
					$embed = preg_replace('@ width="(...)" height="(...)"@', ' width="340" height="255" wmode="opaque"', $embed);
					// CORRIGIR TRANSPARENT DO IFRAME DO YOUTUBE: MALDITO!!!!!! @link http://stackoverflow.com/a/4788044/679195
					$embed = str_replace( '?', '?wmode=opaque&', $embed);
					echo "<p>{$embed}</p>";
				}
				?>
				<?php echo apply_filters( 'the_content', $instance['html'] ); ?>
			</div>
		</div>
		<?php
		echo $after_widget;
		...
		?>
			<div class="free_box_form">
				...
				<p>
					...
					<input type="text" id="<?php echo $this->get_field_id('titulo'); ?>" name="<?php echo $this->get_field_name('titulo'); ?>" value="<?php echo $instance['titulo']; ?>" class="iptw_full" />
				</p>
				<div>
				...
				<p>
					...
					<input type="text" id="<?php echo $this->get_field_id('link'); ?>" name="<?php echo $this->get_field_name('link'); ?>" value="<?php echo $instance['link']; ?>" class="iptw_full" />
				</p>
				...
				<p>
					...
					<textarea name="<?php echo $this->get_field_name('html'); ?>" id="<?php echo $this->get_field_id('html'); ?>" class="iptw_full  ipt_textarea form_element form_element_height_<?php echo $editor_attr['height']; ?> form_textarea form_text_editor editor_type_<?php echo $editor_attr['editor_type']; ?>"><?php echo format_to_edit(wpautop($instance['html'])); ?></textarea>
				</p>
				<p>
					<label for="<?php echo $this->get_field_id('video'); ?>">Endereço do vídeo:</label><br />
					<input type="text" id="<?php echo $this->get_field_id('video'); ?>" name="<?php echo $this->get_field_name('video'); ?>" value="<?php echo $instance['video']; ?>" class="iptw_full" />
				</p>
			</div>
		<?php
	}
